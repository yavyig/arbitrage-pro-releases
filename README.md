🚀 Arbitrage Pro
Arbitrage Pro is a high-performance, premium-grade Amazon Arbitrage Scraper designed for speed, accuracy, and ease of use. Built with a modern Electron frontend and a robust Node.js backend, it provides a seamless workflow for professional sellers to find profitable opportunities in seconds.

✨ Features
💎 Premium User Experience
Modern Branding: A sleek, high-contrast "Pro" aesthetic with a two-tone title design and unified color palette.

Interactive Controls: Scraper buttons feature dynamic "Active States," glowing badges, and slide-in animations to guide your workflow.

Smart Terminal: A true stream-emulated terminal that handles high-speed data bursts without splitting lines, complete with a "Cyan Pulse" indicator when a process is active.

Custom Modals: Native Windows alerts replaced with beautiful, theme-consistent dark mode modals for login, updates, and process management.

🛠️ Powerful Functionality
Triple-Threat Scraper Engine: Dedicated modules for Item Scraping, Seller Scraping, and Book Depot Scraping.

Persistent Storage: Customize your results directory once; the app securely remembers your folder location across restarts.

Auto-Login & Security: Securely stores your Serial Key locally after the first activation for instant access on startup.

Seamless Updates: Integrated manual and automatic update check system to ensure you're always running the latest version.

🚀 Getting Started
Prerequisites
Windows 10 or higher

An active Arbitrage Pro Serial Key

Installation
Download the latest Arbitrage-Pro-Setup-1.1.2.exe from the releases page.

Run the installer. The app will automatically create a desktop shortcut and launch.

Enter your Serial Key on the splash screen to authenticate via the secure MongoDB cluster.

⌨️ Using the Terminal
The terminal is the heart of Arbitrage Pro.

Input: Simply type your keywords or worker limits into the input box and hit Enter or click Send.

Auto-Scroll: The terminal automatically snaps to the bottom, but you can scroll up to review logs; a "Jump to Bottom" arrow appears if you're looking at history.

Visual Feedback: The terminal border glows Cyan when a scraper is actively running.

🛠️ Built With
Frontend: HTML5, CSS3 (Flexbox/Gradients), JavaScript (Renderer/Preload).

Backend: Electron, Node.js, Mongoose.

Automation: Playwright Extra with Stealth plugins for undetected scraping.

Security: Node-Machine-Id for hardware-locked licensing.

👤 Support
Having trouble or need to extend your license?

Contact: Reach out on Instagram @yavuzalper or via the Contact button in the app navigation bar.

© 2026 Arbitrage Pro. All rights reserved.
